...
#@-node:ekr.20050724074642.23:Overridden methods
#@-node:ekr.20050724074619.1:Birth (From usetemacs)
#@+node:ekr.20050724075352.13:Helper classes (temacs)
#@+others
#@+node:ekr.20050724075352.14:class ControlXHandler  TO DO: remove these tables.
class ControlXHandler:
    ...
    def processKey( self, event ):
        ...
        if ( event.keysym == 'r' and emacs.rectanglemode == 0 ) and not emacs.registerCommands.registermode:
...
class MC_StateManager:
    ...
    #@nonl
    #@-node:ekr.20050724075352.23:__call__
    #@+node:ekr.20050725112958:setStateCommands TO BE REMOVED
...
class Emacs:
    ...
    def __init__ (self,c,tbuffer=None,minibuffer=None,useGlobalKillbuffer=False,useGlobalRegisters=False):
        ...
        self.negArgs ={
            '<Alt-c>':self.editCommands.changePreviousWord, 
            '<Alt-u>':self.editCommands.changePreviousWord, 
            '<Alt-l>':self.editCommands.changePreviousWord,
        } # For negative argument functionality
    ...
    #@nonl
    #@-node:ekr.20050724075352.41:Emacs.__init__
    #@+node:ekr.20050727145756:Init & configurations
    ...
    def setBufferStrokes (self,tbuffer,label):
        ...
        def cb (evstring):
            ...
            if evstring!='<Key>':
                # g.trace(evstring)
                tbuffer.bind(evstring,lambda event,meth=_cb:self.keyHandler.masterCommand(event,meth,evstring))
            else:
                # g.trace('+',evstring)
                tbuffer.bind(evstring,lambda event,meth=_cb:self.keyHandler.masterCommand(event,meth,evstring),'+')
        #@nonl
    ...
    #@-node:ekr.20050724075352.109:undoer methods
    #@-node:ekr.20050727145756:Init & configurations
    #@+node:ekr.20050724075352.48:Dictionaries...
    #@+at
    ...
    def addCallBackDict( self ):
        ...
        ### This can be replaced by Leo's normal menu-creation logic.
        
    
        '''This method adds a dictionary to the Emacs instance through which the masterCommand can call the specified method.'''
    
        cbDict = {
        'Alt-less' : lambda event, spot = '1.0' : self.editCommands.moveTo( event, spot ),
        'Alt-greater': lambda event, spot = 'end' : self.editCommands.moveTo( event, spot ),
        'Control-Right': lambda event, way = 1: self.editCommands.moveword( event, way ),
        'Control-Left': lambda event, way = -1: self.editCommands.moveword( event, way ),
        'Control-a': lambda event, spot = 'insert linestart': self.editCommands.moveTo( event, spot ),
        'Control-e': lambda event, spot = 'insert lineend': self.editCommands.moveTo( event, spot ),
        'Alt-Up': lambda event, spot = 'insert linestart': self.editCommands.moveTo( event, spot ),
        'Alt-Down': lambda event, spot = 'insert lineend': self.editCommands.moveTo( event, spot ),
        'Alt-f': lambda event, way = 1: self.editCommands.moveword( event, way ),
        'Alt-b' : lambda event, way = -1: self.editCommands.moveword( event, way ),
        'Control-o': self.editCommands.insertNewLine,
        ...
        'Control-s' : None,
        'Control-r' : None,
        'Alt-c': lambda event, which = 'cap' : self.editCommands.capitalize( event, which ),
        'Alt-u': lambda event, which = 'up' : self.editCommands.capitalize( event, which ),
        'Alt-l': lambda event, which = 'low' : self.editCommands.capitalize( event, which ),
        'Alt-t': lambda event, sw = self.editCommands.swapSpots: self.editCommands.swapWords( event, sw ),
        'Alt-x': self.alt_X,
        'Control-x': self.startControlX,
        'Control-g': self.keyboardQuit,
        'Control-Shift-at': self.editCommands.setRegion,
        'Control-w': lambda event, which = 'd' :self.editCommands.killRegion( event, which ),
        'Alt-w': lambda event, which = 'c' : self.editCommands.killRegion( event, which ),
        'Control-t': self.editCommands.swapCharacters,
        'Control-u': None,
        'Control-l': None,
        'Alt-z': None,
        'Control-i': None,
        'Alt-Control-backslash': self.editCommands.indentRegion,
        'Alt-m' : self.editCommands.backToIndentation,
        'Alt-asciicircum' : self.editCommands.deleteIndentation,
        'Control-d': self.editCommands.deleteNextChar,
        'Alt-backslash': self.editCommands.deleteSpaces, 
        'Alt-g': None,
        'Control-v' : lambda event, way = 'south': self.editCommands.screenscroll( event, way ),
        'Alt-v' : lambda event, way = 'north' : self.editCommands.screenscroll( event, way ),
        'Alt-equal': self.editCommands.countRegion,
        'Alt-parenleft': self.editCommands.insertParentheses,
        'Alt-parenright': self.editCommands.movePastClose,
        'Alt-percent' : None,
        'Control-c': None,
        ...
        'Control-Alt-w': None,
        'Alt-a': self.editCommands.backSentence,
        'Alt-e': self.editCommands.forwardSentence,
        'Control-Alt-o': self.editCommands.insertNewLineIndent,
        'Control-j': self.editCommands.insertNewLineAndTab,
        'Alt-minus': self.negativeArgument,
        'Alt-slash': self.editCommands.dynamicExpansion,
        'Control-Alt-slash': self.editCommands.dynamicExpansion2,
        ...
        'Alt-q': self.editCommands.fillParagraph,
        'Alt-h': self.editCommands.selectParagraph,
        'Alt-semicolon': self.editCommands.indentToCommentColumn,
        ...
        'Alt-s': self.editCommands.centerLine,
        'Control-z': self.emacsControlCommands.suspend, 
        ...
        'Alt-exclam': self.emacsControlCommands.startSubprocess,
        'Alt-bar': lambda event: self.emacsControlCommands.startSubprocess( event, which = 1 ),
        }
        
        return cbDict
    #@nonl
    #@-node:ekr.20050724075352.49:addCallBackDict (creates cbDict) TO BE REMOVED
    #@+node:ekr.20050724075352.50:addXCommands TO BE REMOVED
    def addXCommands( self ):
        
        xcommands = {
            '<Control-t>': self.editCommands.transposeLines, 
            ...
            '<Control-o>': self.editCommands.removeBlankLines,
            '<Control-i>': self.fileCommands.insertFile,
            '<Control-s>': self.fileCommands.saveFile,
            '<Control-x>': self.editCommands.exchangePointMark,
            '<Control-c>': self.emacsControlCommands.shutdown,
            '<Control-b>': self.bufferCommands.listBuffers,
            '<Control-Shift-at>': lambda event: event.widget.selection_clear(),
            '<Delete>' : lambda event, back = True: self.editCommands.killsentence( event, back ),
        }
    ...
    #@-node:ekr.20050724075352.50:addXCommands TO BE REMOVED
    #@-node:ekr.20050724075352.48:Dictionaries...
    #@+node:ekr.20050727145756.1:Key handling...
    ...
    def stopControlX( self, event ):
        ...
        if self.emacsControlCommands.shuttingdown: return
        self.rectangleCommands.sRect = False
        self.mcStateManager.clear()
        event.widget.tag_delete( 'color' )
        event.widget.tag_delete( 'color1' )
        if self.registerCommands.registermode:
    ...
    #@-node:ekr.20050724075352.313:startEvaluate
    #@-node:ekr.20050724075352.310:Esc methods for Python evaluation ???
    #@+node:ekr.20050724075352.53:Utilities
    #@+at
    ...
    class macroCommandsClass (baseCommandsClass):
        ...
        #@-node:ekr.20050724075352.148:saveMacros & helper
        #@-node:ekr.20050727085306.1:Entry points
        #@+node:ekr.20050727085306.2:Called from keystrok handlers
    ...
    class keyHandlerClass:
        ...
        def __init__ (self,emacs,altXCommands):
            ...
            self.doAltX = self.addAltXCommands() ### This will go away.
            self.doAltX.update(altXCommands) 
        
            # Delegators.
            self.mcStateManager = emacs.mcStateManager
            self.kstrokeManager = emacs.kstrokeManager
        ...
        #@nonl
        #@-node:ekr.20050727162112:ctor
        #@+node:ekr.20050724075352.51:addAltXCommands (TO BE REMOVED)
        def addAltXCommands( self ):
            ...
            doAltX= {       
                # Args.
                'digit-argument': lambda event: self.emacs.universalDispatch( event, '' ),
                'universal-argument': lambda event: self.emacs.universalDispatch( event, '' ),   
        
                # Eval.
                'repeat-complex-command': lambda event: self.emacs.repeatComplexCommand( event ),
                'eval-expression': self.emacs.startEvaluate,
            }
        ...
        #@nonl
        #@-node:ekr.20050724075352.51:addAltXCommands (TO BE REMOVED)
        #@+node:ekr.20050724075352.43:masterCommand (temacs)
