...
"""Runs a program and interface Leo through its input/output/error streams - @run 'cmd' - @in 'msg' or redirect input and output."""
...
# 
# - Double clicking on the icon of an @run <command> node will launch 
# <command>, it will also mark the node.
# 
...
# infinite number of arguments to <command>. The output of the program is 
# written in the log pane (Error outputed in red). When the program exit the 
# node is set unmarked and the return value is displayed... When the enter key 
# is pressed in the body pane of an active @run node the content of it body 
# pane is written to the program and then emptied ready for another line of 
# input. If the node have @run nodes in its descendance, they will be launched 
# successivelly. (Unless one returned an exit code other than 0, then it will 
# stop there)
# 
